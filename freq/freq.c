...
main(){
	...
	double cpufreq;
	printf("(freq) Test begin. Please use <ctrl> + <C> to halt.\n\n");
	while(1){
		...
		printf("(freq) The cpu frequency is %.2f MHz, time = %ld us, cycle = %llu\n",cpufreq,tv2.tv_sec - tv1.tv_sec,cycle_2 - cycle_1);
	}
	return;
}
